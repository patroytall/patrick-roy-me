...
Absolutes can be applied at multiple levels: organization, division, project, or team. The limit on freedom caused by enforcing an absolute must be carefully assessed especially at the higher levels. IBM is the company I know who enforced the most absolute standards at the organization level. This was referred to as being blue or blue washed when you were acquired. Because IBM was competing in almost every market it felt it had to use its own technology for everything. This resulted in painful productivity deficiencies. I say painful to underline the fact people knew work could be accomplished more effectively but could not because IBM dictated its standards. In some areas, like single sign-on, it paid off but in most it was a frustrating experience. My take on it is that absolutes should rise bottom up because they are believed to be the correct path. For example, by professionals sharing similar interests in guilds.

Enforcing absolutes is a hard task. Even when well communicated, adopting a standard requires coordination and dedication. The goal is to create a movement toward the objective. To get champions to propagate practices which augments the organization. In a previous organization, we had two primary development tools for the same language. The split was about 50/50. Even though one of the tools was clearly superior, it took 15 months to adopt it; one developer at a time voluntarily. The gain was substantial in the long run as only one set of configuration and plugins needed to be supported.
...
An absolute is great when there are no exceptions but even if your absolute scenario only brings the number of exceptions down it is still a win. Once the count is reduced, it will allow you to document them to make it a semi-absolute.
...
Code formatting has triggered debates throughout computer programming history. Individuals have not relinquished their individuality for years in the name of betterness. Code formatters have evolved and become a blessing but still you can configure them to your project liking which leads to debates.
...
When coding, there are often multiple ways to express the same logic. An approach is to always use the expressions with the least syntactic elements. It might not be ideal in some cases but you will not spend time wondering which one to use. The team will align to the convention rapidly because of its simplicity.
...
```
A complete reorganization of the code could likely have avoided the awful condition in the first place. Thus allowing for extra brackets to make things clearer can encourage poor coding practices.
...
When acronyms are used in this convention people tend to write them in upper case: `siteURL` or `parseHTML`. A problem occurs when an acronym comes first or when there are two consecutive acronyms. `HTMLand` looks like HTML but is really HTM followed by land. I experienced a similar case which took us three weeks to debug because our brain was fooled by the syntax. `HTMLURL` is obviously difficult and non-deterministic.
...
An example of a test relying on timing, is a test writing to an eventually consistent database which reads the value later. This is just one of many patterns of timing dependencies for tests.
...
Separate your slow timing dependent tests from your fast and deterministic ones. False positive investigations are expensive. Non deterministic tests are like mine in a field. You always have to worry about stepping on one. The partitioning will save you costly debugging.

The value of tests is based on numbers. Deleting a few does not have a significant impact on quality.

It is often possible to mock time to implement time dependent tests. This is a great solution since it makes the tests fast and deterministic without losing much accuracy.

## Sum it up

You are driving on the U.S. Route 50 across the desert in a convertible. The wind is refreshing. The road is straight without crossroads. You are cruising way above the speed limit. The sun rising on the left tells you it is going to be a beautiful day.
